...
@interface QJCircleView () <UIGestureRecognizerDelegate>
{
...
//        _trackLayer.opacity = 0.25;//背景圆环的背景透明度
        ...
        _label = [[UILabel alloc] initWithFrame:CGRectMake(width*.1, height/2 - width/6, 85, 25)];
        self.label.center = CGPointMake(self.trueCenter.x, self.trueCenter.y+20);
        ...
        //图片
        _iconIM = [[UIImageView alloc] initWithFrame:CGRectMake(width*.1, height/2 - width/6, 40, 40)];
        self.iconIM.center = CGPointMake(self.trueCenter.x, self.trueCenter.y-15);
        [self addSubview:self.iconIM];
...
- (void) willMoveToSuperview:(UIView *)newSuperview{
    ...
    // label
//    self.label.text = [NSString stringWithFormat:@"%d %@", self.currentNum, self.units];
    
    //图片设置
    self.iconIM.image = [UIImage imageNamed:self.iconName];
...
- (UIBezierPath *)createArcPathWithAngle:(double)angle atPoint: (CGPoint) point withRadius: (float) radius
{
    ...
    UIBezierPath *aPath = [UIBezierPath bezierPathWithArcCenter:point radius:radius startAngle:DEGREES_TO_RADIANS(270) endAngle:DEGREES_TO_RADIANS(endAngle) clockwise:YES];
...
{
    self.trackLayer.strokeColor = maximumTrackTintColor.CGColor;
}

//中间颜色
- (void)setColor:(UIColor *)color
{
    self.trackLayer.fillColor = color.CGColor;
}

//设置图片
- (void)setIconName:(NSString *)iconName
{
    self.iconIM.image = [UIImage imageNamed:iconName];
}
