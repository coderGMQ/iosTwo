...
- (UISegmentedControl *)segmentView{
    ...
    if (!_segmentView) {
        ...
        _segmentView = [[UISegmentedControl alloc] initWithItems:@[@"开始时间",@"结束时间"]];
    }
    return _segmentView;
}
...
- (void)setType:(int)type{
    ...
    if (1 == type) {
        ...
        //隐藏
        self.label.text = @"  ";
        self.segmentView.frame = CGRectMake(kFitW(50), self.label.y - kFitW(3), kWidth - kFitW(50 * 2), self.label.h + kFitW(6));
        [self.label.superview addSubview:self.segmentView];
...
-(void)setupUI {
    ...
    self.label.textColor = kOrangeColor;
...
-(void)addLabelWithName:(NSArray *)nameArr {
    ...
    for (int i=0; i<nameArr.count; i++) {
        CGFloat labelX = kPickerSize.width/(nameArr.count*2)+18+kPickerSize.width/nameArr.count*i;
         UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(labelX + 1, (self.showYearView.frame.size.height - 18)/2, 18, 18)];
...
-(void)dismiss {
    ...
    //类型为1且选择时间为开始时间时候，不执行以下代码
    if (1 == self.type && 0 == self.dateType) {
